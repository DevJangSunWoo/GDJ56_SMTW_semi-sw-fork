...
public class ReviewCommentsDao {
	...
	private ReviewComments getReviewComments(ResultSet rs) throws SQLException {
		return ReviewComments.builder().rcNo(rs.getInt("RC_NO")).reviewCommentLevel(rs.getInt("REVIEW_COMMENT_LEVEL"))
				.reviewCommentRef(rs.getInt("REVIEW_COMMENT_REF")).rcContents(rs.getString("RC_CONTENTS"))
				.enrollDate(rs.getDate("ENROLL_DATE")).reviewNo2(rs.getInt("REVIEW_NO2"))
				.memberId(rs.getString("MEMBER_ID")).qcCount(rs.getInt("count")).build();
	}
	...
	public int insertReviewComments(Connection conn, int qcLevel, String qnaQcRef, String comment, int qnaNo,
			String writer) {
		PreparedStatement pstmt = null;
		int result = 0;
		try {
