...
public class ReviewDao {
	...
	public ReviewDao() {
		String path = ReviewDao.class.getResource("/sql/review/reviewsql.properties").getPath();
		try {
	...
	private Review getReview(ResultSet rs) throws SQLException {
		return Review.builder().reviewNo(rs.getInt("REVIEW_NO")).memberId(rs.getString("MEMBER_ID"))
				.nationName(rs.getString("N_NAME")).reviewTitle(rs.getString("REVIEW_TITLE"))
				.reviewContnet(rs.getString("REVIEW_CONTENTS")).enrollDate(rs.getDate("ENROLL_DATE"))
				.reviewCity(rs.getString("REVIEW_CITY")).reviewSat(rs.getInt("REVIEW_SAT"))
				.reviewFileName(rs.getString("FILENAME")).build();
	}
	...
	public List<Review> searchReviewAll(Connection conn, int cPage, int numPerpage, String stateSort) {
		...
		try {
			// 기존 쿼리문
			// SELECT * FROM (SELECT rownum AS rnum, R.* FROM (SELECT * FROM REVIEW)R) WHERE
			// rnum BETWEEN ? AND ?
			// 변경하고 싶은 쿼리문
//			SELECT * FROM 
...
//			WHERE rnum BETWEEN 3 AND 5;
	...
	public List<Review> searchReviewList(Connection conn, String type, String keyword, int cPage, int numPerpage,
		...
		/// 여기까지가 SELECT * FROM(SELECT ROWNUM AS RNUM,R.*FROM(SELECT *FROM REVIEW WHERE
		/// $COL LIKE ?
		String addQuery = "";
		if (stateSort != null) {
			addQuery += " " + stateSort + " ";
	...
	public int selectReviewCount(Connection conn, String type, String keyword) {
		...
		try {
			...
			rs = pstmt.executeQuery();
			if (rs.next())
				result = rs.getInt(1); // 1 대신 count(*) 여도 상관없음 성별 아이디
		} catch (SQLException e) {
			e.printStackTrace();
		} finally {
			close(rs);
