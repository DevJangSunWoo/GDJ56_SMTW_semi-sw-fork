...
public class EnrollReviewEndServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (!ServletFileUpload.isMultipartContent(request)) {
			...
			// 객체 생성
			Review r = Review.builder().memberId(userId).reviewTitle(reviewTitle).nationName(reviewNation)
					.reviewCity(reviewCity).reviewSat(reviewSat).reviewFileName(fileName).reviewContnet(content)
					.build();
