...
public class ReviewBasicServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (pageNo == 1) {
			...
			pageBar += "<li><a class='page-link' href='" + request.getContextPath() + "/community/reviewBasic.do?cPage="
					+ (pageNo - 1) + "' style='color:rgba(221, 160, 221, 0.508) !important;'>이전</a></li>";
		}

		while (!(pageNo > pageEnd || pageNo > totalPage)) {
			if (cPage == pageNo) {
				pageBar += "<li class='page-item'><a class='page-link' style='color:rgba(221, 160, 221, 0.508) !important;'>"
						+ pageNo + "</a></li>";
			} else {
				pageBar += "<li class='page-item'><a class='page-link' href='" + request.getContextPath()
						+ "/community/reviewBasic.do?cPage=" + pageNo + "&stateSort=" + stateSort
