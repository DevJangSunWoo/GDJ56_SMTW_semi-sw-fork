...
public class SearchReviewServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (pageNo == 1) {
			...
			pageBar += "<a href='" + request.getContextPath() + "/community/searchReview.do?cPage=" + (pageNo - 1)
					+ "&searchType=" + type + "&searchKeyword=" + keyword + "&stateSort=" + stateSort + "'>" + pageNo
					+ "</a>";
		}
		...
		while (!(pageNo > pageEnd || pageNo > totalPage)) {
			if (cPage == pageNo) {
				...
				pageBar += "<a href='" + request.getContextPath() + "/community/searchReview.do?cPage=" + pageNo
						+ "&searchType=" + type + "&searchKeyword=" + keyword + "&stateSort=" + stateSort + "'>"
						+ pageNo + "</a>";
			}
