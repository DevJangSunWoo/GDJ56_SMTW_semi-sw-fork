...
<link rel="stylesheet" type="text/css"
	href="<%=request.getContextPath()%>/css/readReview.css" />
<link
...
<section style="height: auto;">
	<div
		style="border: 1px solid; width: 100%; height: 150px; text-align: center; font-size: x-large; justify-content: center; display: flex;">
		<div style="margin-top: 50px">
			<%=r.getReviewTitle()%>
		</div>
	</div>
	<div
		style="background-color: rgba(128, 0, 128, 0.4); width: 100%; height: 3px;"></div>
	<div style="width: 100%; height: 7px;"></div>
	<div
		style="display: flex; border: 0px solid rgb(15, 231, 231); width: 100%; height: 75px">
		<div
			style="border: 0px solid yellow; width: 500px; height: 60px; display: inline-flex; align-items: center;">
			<div>
		...
		<div
			style="border: 0px solid yellow; width: 400px; height: 60px; margin-left: 50px;">
			<div class="d-grid gap-2 d-md-flex justify-content-md-end">
				<%
				if (logInMember != null
						&& (logInMember.getMemberId().equals("ADMIN") || logInMember.getMemberId().equals(r.getMemberId()))) {
				%>
				<button
					onclick="update_check('<%=r.getReviewNo()%>','<%=r.getReviewFileName()%>');"
					class="btn btn-primary customBtn btnStyle" type="button"
					style="background-color: rgba(221, 160, 221, 0.508) !important;"
					value="수정">수정하기</button>
				<button
					onclick="delete_check('<%=r.getReviewNo()%>','<%=r.getReviewFileName()%>');"
					class="btn btn-primary customBtn btnStyle" type="button"
					style="background-color: rgba(221, 160, 221, 0.479) !important;"
					value="삭제">삭제하기</button>
				<%}%>
			</div>
		</div>
	</div>
	...
	<div style="height: 700px">
		<div style="display: flex; border: 1px;">
			<div
				style="border: 0px solid red; width: 100%; height: 695px; display: flex;">
				<div style="display: flex">
					<div
						style="border: 0px solid green; width: 1390px;; height: 690px;">
						<div
							style="border: 0px solid pink; width: 950px; height: 110px; margin-left: 250px;">
						...
						<div
							style="border: 0px solid pink; width: 950px; height: 550px; text-align: center; margin-left: 0px; margin-top: 30px;">
							<div
								style="border: 1px solid rgba(137, 43, 226, 0.247); width: 950px; height: 800x; text-align: left; border-radius: 20px; margin-left: 250px;">
								<img
									src="<%=request.getContextPath()%>/upload/review/<%=r.getReviewFileName()%>"
									alt=""
									style="width: 50%; height: 50%; margin: auto; display: block; border-radius: 20px;">
							</div>
							<div class="product-title"
								style="border: 1px solid rgba(137, 43, 226, 0.301); width: 950px; height: auto; text-align: center; border-radius: 20px; margin-left: 250px;">
								<div class="product-img-div" style="width: 100%;"
									style="height:auto;">
									<p>
										<%=r.getReviewContnet()%>
									</p>
							...
							<!--영역  -->
							<div
								style="border: 0px solid rgba(137, 43, 226, 0.247); width: 1400px; height: 800x; text-align: left; border-radius: 20px; margin-right: 800px;">
								<div
									style="display: flex; border: 0px solid rgb(15, 231, 231); width: 100%; height: 75px">
									<div
										style="border: 0px solid yellow; width: 500px; height: 60px"></div>
									<div
										style="border: 0px solid yellow; width: 500px; height: 60px"></div>
									<div
										style="border: 0px solid yellow; width: 400px; height: 60px; margin-left: 50px;">
								...
								<div class="comment-editor"
									style="border: 0px solid green; width: 850px; height: auto; margin: 0 auto; margin-top: 50px;">
									<form class="form comment-form"
										style="display: flex; flex-direction: column; align-items: center;"
										action="<%=request.getContextPath()%>/community/insertRC.do"
										onsubmit="return insertComment();">
										<textarea id="comment_1" onclick="logInCheck();"
											name="comment_1" class="commentText" placeholder="댓글을 남겨보세요"
											style="width: 100%;"></textarea>
										<input type="hidden" name="qnaNo" value="<%=r.getReviewNo()%>">
										<input type="hidden" name="qcLevel" value="1" />
										<!-- 댓글레벨  -->
										<input type="hidden" name="qnaQcRef" value="0" />
										<!-- 답글이 아니라 그냥 댓글이라서 0 -->
										<input type="hidden" id="commentWriter" name="commentWriter"
											value="<%=logInMember != null ? logInMember.getMemberId() : ""%>">
										<!-- 댓글작성자 아이디 넘기기  -->
										<div style="width: 100%;">
											<button type="submit" class="submit customBtn btnStyle"
												id="btn-insert"
												style="width: 80px; height: 47px; float: right;">댓글등록</button>
										</div>
									</form>
								</div>
								<!-- 등록된 댓글 -->
								...
								<div class="comments level1"
									style="border: 0px solid green; width: 850px; height: auto; margin: 0 auto;">
									<div class="comment replies"
										style="border: 5px solid #ddd; width: 850px; height: auto; margin: 0 auto; background-color: #f2f2f2; margin-bottom: 10px; margin-top: 10px;">
										<div class="content">
											<header class="top">
												<div class="username" style="color: #7e8cd2"><%=rc.getMemberId()%></div>
												<div class="utility">
													<!-- 현재버튼의 위치 -->
													<button class="menu btn-reply customBtn btnStyle"
														onclick="btnReply(event);">답글등록</button>
													<!-- 관리자와 해당 작성자만 보이게 -->
													<%if (logInMember != null
															&& (logInMember.getMemberId().equals("ADMIN") || logInMember.getMemberId().equals(rc.getMemberId()))) {
													%>
													<button class="menu customBtn btnStyle"
														onclick="deleteComment('<%=rc.getRcNo()%>');">댓글삭제</button>
													<%}%>
												</div>
											</header>
											...
											<ul class="bottom">
												...
												<li class="divider"></li>
												<%if (rc.getQcCount() != 0) {%>
												<li class="menu show-reply" onclick="showReply(event);"
													style="cursor: pointer;">답글 (<%=rc.getQcCount()%>)
												</li>
												<%}%>
											</ul>
										</div>
									...
									<!-- 답글  -->
									<form id="refly-Form" class="form reply-form"
										style="display: none; flex-direction: column; align-items: center;"
										action="<%=request.getContextPath()%>/community/insertRC.do"
										onsubmit="return checkReply(event);">
										<textarea id="replyText" name="comment_1" class="commentText"
											onclick="logInCheck();" placeholder="답글을 남겨보세요"
											style="width: 100%;"></textarea>
										<input type="hidden" name="qnaNo" value="<%=r.getReviewNo()%>">
										<input type="hidden" name="qcLevel" value="2" />
										<!-- 댓글레벨 2 -->
										<input type="hidden" name="qnaQcRef" value="<%=rc.getRcNo()%>" />
										<!-- 답글이라서 해당 댓글번호 -->
										<input type="hidden" id="commentWriter" name="commentWriter"
											value="<%=logInMember != null ? logInMember.getMemberId() : ""%>">
										<!-- 댓글작성자 아이디 넘기기  -->
								...
								<div class="replies level2"
									style="border-left: 5px solid #ddd; border-right: 5px solid #ddd; border-bottom: 5px solid #ddd; width: 850px; height: auto; margin: 0 auto; display: none;">
									<div class="reply">
										<div class="content">
											<header class="top">
												...
												<div class="utility">
													...
													%>
													<button class="menu customBtn btnStyle"
														onclick="deleteQC('<%=rc.getRcNo()%>');">답글삭제</button>
													<%}%>
												</div>
											</header>
											<p><%=rc.getRcContents()%></p>
								...
								<%}%>
								<div
									style="border: 0px solid yellow; width: 100%; height: 30px;"></div>
								<div
									style="border: 0px solid yellow; width: 100%; height: 20px;">
								</div>
								<div
									style="border: 0px solid yellow; width: 100%; height: 20px;">
								</div>
								<div
									style="background-color: rgba(128, 0, 128, 0.4); width: 100%; height: 3px;"></div>
								<div
									style="border: 0px solid yellow; width: 100%; height: 30px;">
									<button type="button" class="submit customBtn btnStyle">
										<a
											href="<%=request.getContextPath()%>/community/reviewBasic.do"><p>목록</p></a>
									</button>
								</div>
							</div>
						</div>
